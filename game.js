...
class Player {
    constructor(id, name) {
        ...
        this.chips = 1000;
        this.currentBet = 0;
        this.totalBetThisHand = 0; // Общая сумма поставленная за всю руку
        this.hasActed = false;
        this.isFolded = false;
        this.isAllIn = false;
    }
    ...
    evaluateBestHand(cards) {
        ...
        for (let combo of combinations) {
            ...
            if (handRank.rank > bestRank ||
                (handRank.rank === bestRank && this.compareHands(handRank, bestHand) > 0)) {
                bestHand = handRank;
    ...
    // Получить все возможные комбинации из n карт
    getAllCombinations(cards, n) {
    ...
    evaluateHand(cards) {
        ...
        if (isFlush && isStraight) {
            return { rank: HAND_RANKINGS.STRAIGHT_FLUSH, description: 'Стрит Флеш', high: Math.max(...values), cards: sortedCards };
        }

        if (counts[0] === 4) {
            const fourKind = uniqueValues.find(v => valueCounts[v] === 4);
            const kicker = uniqueValues.find(v => valueCounts[v] === 1);
            return { rank: HAND_RANKINGS.FOUR_OF_A_KIND, description: 'Каре', high: fourKind, kicker, cards: sortedCards };
        }

        if (counts[0] === 3 && counts[1] === 2) {
            const trips = uniqueValues.find(v => valueCounts[v] === 3);
            const pair = uniqueValues.find(v => valueCounts[v] === 2);
            return { rank: HAND_RANKINGS.FULL_HOUSE, description: 'Фулл Хаус', high: trips, low: pair, cards: sortedCards };
        }
        ...
        if (counts[0] === 2 && counts[1] === 2) {
            ...
            const kicker = uniqueValues.find(v => valueCounts[v] === 1);
            return { rank: HAND_RANKINGS.TWO_PAIR, description: 'Две Пары', high: pairs[0], low: pairs[1], kicker, cards: sortedCards };
        }
    ...
    compareHands(hand1, hand2) {
        ...
        switch (hand1.rank) {
            ...
            case HAND_RANKINGS.FULL_HOUSE:
                if (hand1.high !== hand2.high) return hand1.high - hand2.high;
                return hand1.low - hand2.low;

            case HAND_RANKINGS.FLUSH:
            case HAND_RANKINGS.HIGH_CARD:
                for (let i = 0; i < hand1.high.length; i++) {
            ...
            case HAND_RANKINGS.TWO_PAIR:
                if (hand1.high !== hand2.high) return hand1.high - hand2.high;
                if (hand1.low !== hand2.low) return hand1.low - hand2.low;
                return hand1.kicker - hand2.kicker;
...
class PokerGame {
    constructor(chatId) {
        ...
        this.bettingRound = 0;
        this.sidePots = []; // Массив сайд-потов
    }
    ...
    // Создание сайд-потов для all-in ситуаций
    createSidePots() {
        const activePlayers = Array.from(this.players.values()).filter(p => !p.isFolded);

        // Сортируем игроков по общей сумме ставок
    ...
    // Определение победителей для showdown
    determineWinners() {
        this.createSidePots();
        const results = [];

        for (let pot of this.sidePots) {
            const activePlayers = pot.eligiblePlayers.filter(p => !p.isFolded);

            if (activePlayers.length === 0) continue;

            if (activePlayers.length === 1) {
                // Только один активный игрок - он забирает пот
            ...
            // Оценка рук всех активных игроков
            ...
            // Находим всех игроков с лучшей рукой (могут быть равные руки)
            const winners = playerHands.filter(ph =>
                ph.player.compareHands(ph.hand, bestHand) === 0
            ).map(ph => ph.player);

            // Делим пот между победителями
    ...
    // Получить строковое представление карт для отображения
    getCardString(card) {
        const values = {
            11: 'J',
            12: 'Q',
    ...
    // Форматирование результатов игры
    formatGameResults(results) {
...
module.exports = { PokerGame, Player };
